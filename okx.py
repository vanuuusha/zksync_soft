...
def get_request_proxies():
    return {"http": settings.okx_proxy, "https": settings.okx_proxy}
...
def get_request_headers(method, endpoint, request_params, body='', sub_account_id=None):
    ...
    mac = hmac.new(settings.secretkey.encode("utf-8"), message.encode("utf-8"), hashlib.sha256)
...
def okx_withdraw(address, amount, num):
    print(f'Пытаюсь вывести ETH с биржи на аккаунт {num}')
    exchange_options = {
        'apiKey': settings.apikey,
        'secret': settings.secretkey,
        'password': settings.okx_passphrase,
    ...
    if amount > min_withdrawal_amount:
        ...
        print(f'Вывожу ETH в количестве {amount+fee} на аккаунт {num}')
    else:
        raise Exception(f'Недостаточко средств аккаует {num} (Пытаюсь вывести {amount+fee})')
    ...
    try:
        ...
        if status == "ok":
            ...
            print(f'Не удалось вывести средства с биржи. Аккаунт {num}. Сумма {amount+fee}')
    except ccxt.InvalidAddress:
        print(f'Адрес {address} аккаунта {num} не добавлен в вайтлист')
        raise Exception(f'Адрес {address} аккаунта {num} не добавлен в вайтлист')
    except ccxt.InsufficientFunds:
        print(f'Для перевода на аккаунт {num}. недостаточно средств, требуется {amount+fee}')
        raise Exception(f'Для перевода на аккаунт {num}. недостаточно средств, требуется {amount+fee}')
    except Exception as e:
        raise Exception(f'Возникла непридвиденная ошибка, вероятно с сетью zksync ведутся технические работы')
